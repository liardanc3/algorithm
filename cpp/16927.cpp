		...
		[&]() {
			for (int i = mny; i <= mxy; i++) {
				for (int j = mnx; j <= mxx; j++) {
					...
					// down
					if (cond3 && !cond2)
						down(i, j, mnx, mxx, mny, mxy, R % (2 * (mxx - mnx + 1) + 2 * (mxy - mny - 1)), arr[i][j]);

					// right
					if (cond2 && !cond4)
						right(i, j, mnx, mxx, mny, mxy, R % (2 * (mxx - mnx + 1) + 2 * (mxy - mny - 1)), arr[i][j]);

					// up
					if (cond4 && !cond1)
						up(i, j, mnx, mxx, mny, mxy, R % (2 * (mxx - mnx + 1) + 2 * (mxy - mny - 1)), arr[i][j]);

					// left
					if (cond1 && !cond3)
						left(i, j, mnx, mxx, mny, mxy, R % (2 * (mxx - mnx + 1) + 2 * (mxy - mny - 1)), arr[i][j]);
				}
			}
		}();
